...
function check_release() {
    ...
    if [ "$RELEASE" == "centos" ]; then
        red "[ × ] 当前暂不支持centos，更多功能正在开发中，感谢支持！"
        exit_program
...
# git clone 确认项目是否已安装、是否需要跳过
check_git_clone() {
echo "[ * ] 正在检查项目包资源，请稍候"
    if [ ! -f "/usr/local/ngrok/Makefile" ]; then # 检查项目包
    echo "[ * ] 未检测到项目包，即将开始下载，请确保网络顺畅"
...
# 确认服务端信息
green "[ - ] 正在确认服务端信息"
echo "[ * ] Tips: 确认服务端信息时，需要确认3个配置信息："
echo "[ * ] 1) 访问域名：绑定在服务端VPS上的域名"
echo "[ * ] 2) 域名端口: 远程域名访问的端口"
echo "[ * ] 3) 访问协议: 远程连接的协议类型。默认为tcp（其他可选: http | https 其他方式请参照官网）"
echo "[ * ] 如：远程访问域名为[testdomain.com]，域名开放的端口为[4443]，访问协议为[tcp]，则输入如下："
echo "[ * ] 访问域名：testdomain.com"
echo "[ * ] 域名端口：4443"
echo "[ * ] 访问协议：tcp"
conf_server_menu3
